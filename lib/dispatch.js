...
function isValidMethod(methodName, model){
    if(typeof model !== 'object'){
        console.error(`model is a ${typeof model}, not an object`)
        return false
    }

    if(typeof methodName !== 'string'){
        console.error('method is not a string')
        return false
    }

    if(!model.hasOwnProperty(methodName) || typeof model[methodName] !== 'function'){
        console.error(`method name ${methodName} does not exist on model`)
        return false
    }
